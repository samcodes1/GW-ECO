...
public interface VoucherRepo extends CrudRepository<VoucherEntity, Long> {
    ...
    @Query(value = "select v.*, c.companyname, c.companylogo from voucher v inner join companies c on c.companyid=v.shopidfk where v.voucherexpiry > CURDATE()", nativeQuery = true)
    List<VoucherProjection> getNonExpiredVouchers();

    @Query(value = "select v.*, c.companyname, c.companylogo from voucher v inner join companies c on c.companyid=v.shopidfk where v.voucherexpiry < CURDATE()", nativeQuery = true)
    List<VoucherProjection> getExpiredVouchers();

    @Query(value = "SELECT count(*) FROM voucher WHERE DATE(vocuhercreatedat) = CURDATE()", nativeQuery = true)
    ...
    @Query(value = "SELECT count(*) FROM voucher WHERE voucherexpiry > CURDATE()", nativeQuery = true)
    Integer countNonexpiredVouchers();
    ...
    @Query(value = "SELECT v.shopidfk,v.usedstatus,v.voucherbarcode,v.voucherid,v.isdiscountpercentage,v.voucherpointscost,v.discountpercentage,v.voucherexpiry,v.vocuhercreatedat,v.description,v.name,c.companyid,c.companyname,c.subscriptionexpiry,c.subscriptiontype,c.company_email,c.companylogo from voucher v inner join companies c on c.companyid=v.shopidfk where v.voucherexpiry < CURDATE() and c.companyid=?1", nativeQuery = true)
    List<VoucherProjection> getCompanyExpiredVouchers(Long companyid); 

    @Query(value = "SELECT v.shopidfk,v.usedstatus,v.voucherbarcode,v.voucherid,v.isdiscountpercentage,v.voucherpointscost,v.discountpercentage,v.voucherexpiry,v.vocuhercreatedat,v.description,v.name,c.companyid,c.companyname,c.subscriptionexpiry,c.subscriptiontype,c.company_email,c.companylogo from voucher v inner join companies c on c.companyid=v.shopidfk where v.voucherexpiry > CURDATE() and c.companyid=?1", nativeQuery = true)
    List<VoucherProjection> getCompanyNonExpiredVouchers(Long companyid); 
